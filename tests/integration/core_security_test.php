...
try {
    ...
    $edit=$payload+['id'=>$documentId,'version'=>1];
    $edit['details']=[3=>lines()[0]];
    ensure(request('/documentos/actualizar',$digitador,$edit)['status']===302,'Remove line from draft');
    ensure(count($docs->find($documentId)['details'])===1,'Removed line persisted');
    ensure(request('/documentos/actualizar',$digitador,$edit)['status']===409,'Stale draft rejected');
    $empty=$edit;$empty['version']=2;$empty['details']=[];
    ensure(request('/documentos/actualizar',$digitador,$empty)['status']===422,'Cannot remove last product');
