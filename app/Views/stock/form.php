...
?>
<section class="module-header"><div><div class="page-eyebrow"><?=$editing?'EDITAR BORRADOR':'NUEVO REGISTRO'?></div><h1 class="page-title"><?=$editing?'Editar stock':'Stock'?></h1><p class="page-subtitle"><?=$editing?'Solo los registros en borrador pueden modificarse.':'Registra un snapshot de inventario por almacén y fecha.'?></p></div><a class="btn btn-outline-primary" href="<?=url('/stock')?>">Volver</a></section>
...
<div class="card-footer bg-white border-0 px-4 pb-4 d-flex flex-wrap gap-2"><button class="btn btn-primary"><?=$editing?'Guardar cambios':'Guardar borrador'?></button><a class="btn btn-outline-primary" href="<?=url('/stock')?>">Cancelar</a></div>
</form>
