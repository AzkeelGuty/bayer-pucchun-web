...
$field=function(string $name,string $label,string $key,mixed $value,string $catalog='',string $type='text',string $extra='')use($catalogs,$errors):void {
    ...
    if($catalog){ echo '<select class="form-select'.($error?' is-invalid':'').'"'.$attrs.'><option value="">Seleccionar…</option>'; foreach($catalogs[$catalog] as $item) echo '<option value="'.e($item['id']).'"'.((string)$value===(string)$item['id']?' selected':'').'>'.e($item['label']).'</option>'; echo '</select>'; }
...
</div><div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mt-4 mb-3"><h2 class="h5 mb-0">Productos</h2><button type="button" class="btn btn-outline-primary" data-add-detail>+ Agregar producto</button></div>
